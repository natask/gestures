...
- Compatibility with both X11 and Wayland ::
  - works on wayland as well as X11 if using the default keystroke generator. The keystroke generator, =evemu_d0= is based on =evemu= and injects keyboard events directly into a connected device. In the future, may create an input device solely for this application.
* Dependencies 
:PROPERTIES:
