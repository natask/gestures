...
- [[#38-todos][TODOS]]
  - [[#59-enable-customization-by-refactoring-code][enable customization by refactoring code.]]
    - [[#commands-for-gesture-start][commands for gesture start]]
    - [[#commands-for-gesture-end][commands for gesture end]]
    - [[#commands-for-touchscreen][commands for touchscreen]]
    - [[#commands-for-gesture-update][commands for gesture update]]
    - [[#rep-rate][rep rate]]
    - [[#add-multi-finger-pinch-gestures][add multi-finger pinch gestures]]
    - [[#detach-implementation-from-personal-workflow][detach implementation from personal workflow]]
    - [[#more-nuanced-application-of-gestures-to-different-attached-devices][more nuanced application of gestures to different attached devices]]
    - [[#add-debugging-notes-about-fixing-config-file-use-online-json-linter-if-the-interal-json-linter-doesnt-lead-to-debug-point][add debugging notes about fixing config file (use online JSON linter if the interal JSON linter doesn't lead to debug point)"]]
  - [[#ask-before-doing-stuff-in-installation-and-uninstallation-scripts][ask before doing stuff in installation and uninstallation scripts]]
  - [[#13-configuration-syntax][configuration syntax]]
    - [[#implement-json-config-file-support][implement JSON config file support.]]
    - [[#use-libinput-gestures-config-file-syntax][use libinput-gestures config file syntax.]]
    - [[#use-fusuma-config-file-syntax][use fusuma config file syntax.]]
  - [[#create-a-standalone-input-device-for-this-application][Create a standalone input device for this application]]
  - [[#enrich-readme][enrich readme]]
  - [[#write-script-to-fulfill-dependencies-automatically][Write script to fulfill dependencies automatically]]
  - [[#include-error-handling-for-mistakes-in-config-file][Include error handling for mistakes in config file]]
  - [[#implement-c-version][Implement C++ version]]
